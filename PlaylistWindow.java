...
/**
 * The playlist GUI window that displays the songs in the queue and the songs in
 * the playlist. The user can accept or reject (>:(<) a song.
 *
 * @author CS 2114
 * @version Spring 2024
 */
...
public class PlaylistWindow
{
    ...
    private static final double DISPLAY_FACTOR = 1.3;
    ...
    private static final int PLAYLIST_SIZE =
        ((int)(500 * DISPLAY_FACTOR) - (4 * PLAYLIST_PADDING)) / 3;
     ...
     * 
     * @param playlistCalculator
     *            a playlist calculator instance to pull queue and playlists
     *            from
     */
    ...
    /**
     * This method dequeues the pending song at the front of the queue, adding
     * it to the playlist (possibly), and updates the window to represent the
     * current status
     *
     * @param button
    ...
    {
        reject.enable();
        if (playlistCalculator.getPlaylistForSong(
            playlistCalculator.getQueue().getFront()) != null)
            accept.enable();
        else
            accept.disable();
    }
    ...
    private void drawPlaylists()
    {
        ...
        drawPhone(
            PLAYLIST_X + (2 * PLAYLIST_SIZE) + (2 * PLAYLIST_PADDING),
            PLAYLIST_Y,
            2);
    }
    ...
    {
        Shape phone = new Shape(
            x,
            y,
            PLAYLIST_SIZE,
            (int)(PLAYLIST_SIZE * 1.25),
            PLAYLIST_COLORS[3]);
        playlistShapes[playlistIndex] = phone;
        Shape innerPhone = new Shape(
            x + 5,
            y + 5,
            PLAYLIST_SIZE - 10,
            (int)(PLAYLIST_SIZE * 1.25) - 10,
            Color.darkGray);
        ...
        TextShape playlistName =
            new TextShape(textX, textY, message, Color.WHITE, 20);
        playlistName.setBackgroundColor(innerPhone.getBackgroundColor());
        playlistName.setX(
            innerPhone.getX()
                + (innerPhone.getWidth() - playlistName.getWidth()) / 2);
        window.addShape(playlistName);
        window.moveToFront(playlistName);
        // populate the playlist with rectangles representing the songs in
        // theplaylist
        int songStartY = playlistName.getY() + playlistName.getHeight() + 5;
        for (int i = 0; i < Math.min(playlist.getNumberOfSongs(), 7); i++)
        {
            Shape song = new Shape(
                innerPhone.getX() + 5,
                songStartY + (i * 25),
                innerPhone.getWidth() - 10,
                20,
                PLAYLIST_COLORS[playlistIndex]);
    ...
    private void drawShapes()
    {
        ...
        for (int i = 0; i < queuedSongs.length; i++)
        {
            ...
            int playlistPreference = playlistCalculator
                .getPlaylistIndex(((Song)queuedSongs[i]).getPlaylistName());
    ...
    private Shape[] drawNote(int x, int y, Color color)
    {
        ...
        Shape circle = new CircleShape(
            x - MUSIC_NOTE_SIZE + bar.getWidth() + 1,
            y + bar.getHeight() - (MUSIC_NOTE_SIZE / 2),
            MUSIC_NOTE_SIZE,
            color);
        note[0] = bar;
        note[1] = circle;
    ...
    /**
     * This method removes the song at the front of the queue and in turn calls
     * updateNotes to redraw the remaining songs (notes) in the queue
     */
    ...
    /**
     * This method redraws the position of the notes on the window. This is
     * invoked whenever the accept or reject button is clicked
     */
    ...
    private void updatePlaylistText()
    {
        for (int i = 0; i < playlistShapes.length; i++)
        {
            ...
            int y =
                playlistShape.getY() + playlistShape.getHeight() + textMargin;
            ...
            popText.setX(
                x + (playlistShape.getWidth() / 2) - (popText.getWidth() / 2));
            ...
            rockText.setX(
                x + (playlistShape.getWidth() / 2) - (rockText.getWidth() / 2));
            ...
            countryText.setX(
                x + (playlistShape.getWidth() / 2)
                    - (countryText.getWidth() / 2));
    ...
    private void endSimulation()
    {
        ...
        window.removeAllShapes();
        // Message
        TextShape simulationStatus =
            new TextShape(0, 0, "All Songs Processed!");
        int x = (window.getGraphPanelWidth() / 2)
            - (simulationStatus.getWidth() / 2);
